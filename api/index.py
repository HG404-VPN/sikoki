...
@app.post("/api/purchase/balance")
def api_purchase_balance():
    data = request.get_json(force=True, silent=True) or {}
    tokens = data.get("tokens")
    package_option_code = data.get("package_option_code")
...
# --- di api/index.py ---
@app.post("/api/purchase/settlement-multi")
def api_purchase_settlement_multi():
    data = request.json or {}
    required = ["tokens", "items"]
    ok = all(k in data for k in required)
    if not ok:
        return jsonify({"error": "missing_fields", "fields": [k for k in required if k not in data]}), 400

    result = purchase_packages_multi(
        api_key=api_key_from_request(request),
        tokens=data["tokens"],
        items=data["items"],
        payment_method=data.get("payment_method", "QRIS"),
        token_payment=data.get("token_payment"),  # boleh kosong; akan diambil otomatis
        ts_to_sign=data.get("ts_to_sign"),        # boleh kosong
        item_name=data.get("item_name", ""),
