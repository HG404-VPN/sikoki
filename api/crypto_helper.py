import os, hmac, hashlib, brotli, zlib, base64
import cloudscraper
...
def ax_api_signature(
    ...
    response = scraper.post(AX_SIGN_URL, json=request_body, headers=headers, timeout=30)
...
def encryptsign_xdata(
    ...
    response = scraper.post(XDATA_ENCRYPT_SIGN_URL, json=request_body, headers=headers, timeout=30)
...
def decrypt_xdata(
    ...
    response = scraper.post(XDATA_DECRYPT_URL, json=encrypted_payload, headers=headers, timeout=30)
...
def get_x_signature_payment(
    ...
    response = scraper.post(PAYMENT_SIGN_URL, json=request_body, headers=headers, timeout=30)
...
def get_x_signature_bounty(
    ...
    response = scraper.post(BOUNTY_SIGN_URL, json=request_body, headers=headers, timeout=30)
    if response.status_code == 200:
        return response.json().get("x_signature")
    else:
        raise Exception(f"Signature generation failed: {response.text}")
