...
def settlement_qris(
    ...
    try:
        decrypted_body = decrypt_xdata(api_key, json.loads(resp.text))
        return decrypted_body  # langsung balikin raw
    except Exception as e:
        print("[decrypt err]", e)
        return {"status": "ERROR", "raw": resp.text, "exception": str(e)}
...
def show_qris_payment(api_key: str, tokens: dict, package_option_code: str,
                      token_confirmation: str, price: int, item_name: str = ""):
    payment_methods_data = get_payment_methods(
        api_key=api_key,
        tokens=tokens,
        token_confirmation=token_confirmation,
    ...
    settlement_res = settlement_qris(
        api_key, tokens, token_payment, ts_to_sign,
        package_option_code, price, item_name
    )

    # langsung return kalau gagal
    if not settlement_res or settlement_res.get("status") != "SUCCESS":
        return settlement_res  # bisa FAILED/ERROR dari XL

    transaction_id = settlement_res["data"]["transaction_code"]

    qris_code = get_qris_code(api_key, tokens, transaction_id)
    if not qris_code:
        return {"status": "FAILED", "error": "Failed to get QRIS code"}
