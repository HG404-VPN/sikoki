...
def purchase_package(api_key: str, tokens: dict, package_option_code: str, amount: int = None) -> dict:
    package_details_data = get_package(api_key, tokens, package_option_code)
    if not package_details_data:
        return {"status": "FAILED", "error": "Failed to get package details for purchase."}
    ...
    if amount is None:
        ...
        if amount_str != "":
            try:
                ...
                return {"status": "FAILED", "error": "Invalid overwrite input"}
    else:
        # API mode → langsung pake field "amount"
        if not isinstance(amount, int):
            return {"status": "FAILED", "error": "Invalid amount type (must be int)"}
        amount_int = amount
    # =======================
    ...
    if payment_res.get("status") != "SUCCESS":
        return {"status": "FAILED", "error": "Failed to initiate payment", "detail": payment_res}
    
    token_payment = payment_res["data"]["token_payment"]
    ts_to_sign = payment_res["data"]["timestamp"]
    
    # --- settlement payload tetap ada ---
    settlement_payload = {
        "total_discount": 0,
        "is_enterprise": False,
        "payment_token": "",
        ...
        "additional_data": {},
        "total_amount": amount_int,   # <--- ini tetep bisa overwrite
...
# --- di api/api_request.py ---
def purchase_packages_multi(
    api_key: str,
    tokens: dict,
    items: list[dict],
    payment_method: str = "QRIS",
    token_payment: str | None = None,
    ts_to_sign: int | None = None,
    item_name: str = "",
    total_amount: int | None = None,
    token_confirmation: str | None = None,
):
    """
    items: list of { "item_code": str, "item_price": int, "item_name": str, "product_type": "", "tax": 0 }
    """

    # --- validasi minimal ---
    if not isinstance(items, list) or len(items) == 0:
        return {"status": "FAILED", "error": "items_required"}

    for i, it in enumerate(items):
        for f in ("item_code", "item_price"):
            if f not in it:
                return {"status": "FAILED", "error": f"items[{i}].{f} missing"}
        # pastikan integer
        it["item_price"] = int(it["item_price"])
        it.setdefault("product_type", "")
        it.setdefault("item_name", item_name or "")
        it.setdefault("tax", 0)

    # total otomatis kalau tidak dipaksa dari argumen
    computed_total = sum(it["item_price"] for it in items)
    amount_int = int(total_amount) if total_amount is not None else computed_total

    if amount_int != computed_total:
        return {
            "status": "FAILED",
            "error": "total_mismatch",
            "detail": {"sum_items": computed_total, "total_amount": amount_int},
        }

    # --- jika caller belum sediakan token_payment & ts_to_sign, ambil via get_payment_methods ---
    if not token_payment or not ts_to_sign:
        ...
        pm = get_payment_methods(
            ...
            tokens=tokens,
            token_confirmation=token_confirmation,
            payment_target=first_code,  # pakai item pertama sbg anchor
        )
    ...
    # --- bangun settlement payload lengkap ---
    settlement_payload = {
        "lang": "id",
        "is_enterprise": False,
        "access_token": tokens["access_token"],
        ...
        "autobuy": {
            "activated_autobuy_code": "",
            "autobuy_threshold_setting": {"label": "", "value": 0, "type": ""},
            "is_using_autobuy": False,
        },
        "can_trigger_rating": False,
        "coupon": "",
        ...
        "verification_token": build_encrypted_field(urlsafe_b64=True),
        "topup_number": "",
        "total_amount": amount_int,
        "total_discount": 0,
        "total_fee": 0,
        # ekstra dari versi minimalis
        "token_payment": token_payment,
        "payment_token": "",
        "activated_autobuy_code": "",
        "cc_payment_type": "",
    ...
    # Bungkus biar sama kayak official format
    payload_dict = {
